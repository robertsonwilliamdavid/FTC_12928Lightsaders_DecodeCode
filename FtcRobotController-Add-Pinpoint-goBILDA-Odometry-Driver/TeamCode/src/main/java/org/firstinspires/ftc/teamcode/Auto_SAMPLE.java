...
@Autonomous(name = "Auto_SAMPLE", group = "Linear Opmode")
public class Auto_SAMPLE extends LinearOpMode {
    //pull cinstants and camera code
    ...
    public void runOpMode() {
        ...
        while (((robo.lift1.getCurrentPosition() < robo.lifthigh) || (runtime.milliseconds() < (drivetimeout)))
            ...
            if ((runtime.milliseconds() < (drivetimeout)) && (((Math.abs(robo.del0.x - robo.location.x) >
                    0.25)) || (Math.abs(robo.del0.y - robo.location.y) > 0.25) || (Math.abs(robo.del0.h -
                    robo.location.h) > 0.25))) {
                XYZ(robo.del0, 8, true, false, 0.2); //0.3
            } else if (resetready) {
                DriveHold(false);
                DriveStop();
                resetotos = true;
        ...
        LeftBarcodeGrab(robo.fgm, 0.4); //0.5
    ...
    public void LeftBarcodeGrab(SparkFunOTOS.Pose2D drivetoposleft, double turnpower) {
        ...
        while ((((robo.lift1.getCurrentPosition() > robo.liftdown) && (runtime.milliseconds() < (drivetimeout))) || grabbing) && opModeIsActive()) {
            ...
            if ((runtime.milliseconds() < (drivetimeout)) && ((!robo.colorseen.matches("Yellow")) &&
                ...
                XYZ(drivetoposleft, 15, true, false, turnpower); //0.7
