...
@Autonomous(name = "Auto_SPECIMEN", group = "Linear Opmode")
public class Auto_SPECIMEN extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        RightCycle(1, 700, false); //650

        RightBarcodeGrab(robo.RgrabL, true, robo.intakewrist0, 10, 1400); //1200

        RightBarcodeGrab(robo.RgrabM, false, robo.intakewrist0, 1, 600);

        RightBarcodeGrab(robo.RgrabR, false, robo.intakewrist25, 1, 1000);

        ServoArm(robo.armlspecdrop, robo.armrspecdrop);
        ClawRot(robo.clawflat);
        droptime = runtime.milliseconds();

        while (runtime.milliseconds() < (droptime + 400)) {
            idle();
        }

        Claw(robo.clawopenfull);

        while (runtime.milliseconds() < (droptime + 500)) {
            idle();
        }
        ...
        ClawRot(robo.clawrotatebackgrab);
            ...
            /*RightCycle(2, 1500,false);

            RightCycle(3, 1500,false);

            RightCycle(4, 1500,false);

            RightCycle(5, 1500,true);*/
    ...
    //region - Right
    public void RightCycle(double cyclecount, double time, boolean last) {
        //region - Right Cycle
        //maybe put into drive to hppen at same time?
        if (cyclecount == 1) {
            ServoArm(robo.armltransfer, robo.armrtransfer);
            ClawRot(robo.clawrotatebar2);
        } else {
            Lift(robo.Lbar2, runtime.milliseconds());
            ServoArm(robo.armltransfer, robo.armrtransfer);
            ClawRot(robo.clawrotatebar2);
            Liftdown(runtime.milliseconds());
        }

        SparkFunOTOS.Pose2D scorepos = robo.barscore;
        if (cyclecount == 2) {
            scorepos.x = scorepos.x - 15;
        } else if (cyclecount == 3) {
            scorepos.x = scorepos.x - 12;
        } else if (cyclecount == 4) {
            scorepos.x = scorepos.x - 9;
        } else if (cyclecount == 5) {
            scorepos.x = scorepos.x - 6;
        }

        drivetimeout = runtime.milliseconds() + time; //2000
        while (opModeIsActive() && (((Math.abs(scorepos.x - robo.location.x) > 0.25)) || (Math.abs(scorepos.y - robo.location.y) > 0.25) ||
                (Math.abs(scorepos.h - robo.location.h) > 0.25)) && (runtime.milliseconds() < drivetimeout)) {
            XYZ(scorepos, 10, true, false, 0.5); //15
        }

        DriveStop();
        Pos(scorepos);

        ServoArm(robo.armlbarpt2, robo.armrbarpt2);
        ...
        if (cyclecount > 1) {
            ...
            while (opModeIsActive() && (((Math.abs(robo.righttrade.x - robo.location.x) > 0.25)) || (Math.abs(robo.righttrade.y - robo.location.y) > 0.25) ||
                ...
                XYZ(robo.righttrade, 25, true, false, 0.5);

                if (runtime.milliseconds() > (drivetimeout - 1900)) { //1800
                    Claw(robo.clawopenfull);
                }

                if (runtime.milliseconds() > (drivetimeout - 1500)) {
                    if (!last) {
                        ServoArm(robo.armlback1, robo.armrback1);
                        ClawRot(robo.clawrotatebackgrab);
                        Claw(robo.clawautograb);
                    } else {
                        ServoArm(robo.armltransfer, robo.armrtransfer);
            ...
            if (!last) {
                ...
                while (opModeIsActive() && (((Math.abs(robo.righttrade2.x - robo.location.x) > 0.25)) || (Math.abs(robo.righttrade2.y - robo.location.y) > 0.25) ||
                    ...
                    XYZ(robo.righttrade2, 25, true, false, 0.5);
    ...
    public void RightBarcodeGrab(SparkFunOTOS.Pose2D grabpos, boolean post1, double intakewristpos, double tolerance, double time) {
        ...
        drivetimeout = runtime.milliseconds() + time;
        driving = true;
        needtodrive = true;
        drop = false;
        stop = true;
        while (opModeIsActive() && (runtime.milliseconds() < (drivetimeout + 1000)) && needtodrive) { //need the +1000?
            if ((((Math.abs(grabpos.x - robo.location.x) > 0.25)) || (Math.abs(grabpos.y - robo.location.y) > 0.25) ||
                    (Math.abs(grabpos.h - robo.location.h) > 1)) && (runtime.milliseconds() < drivetimeout) && needtodrive && driving) {
                XYZ(grabpos, tolerance, true, false, 0.2); //10
                ...
                if (/*((runtime.milliseconds() - stoptime) > 100) &&*/ (!stop)) {
                    if (needtodrive) {
                        driving = false;
                    } else {
                        DriveStop();
            ...
            if (post1) {
                if (runtime.milliseconds() > (drivetimeout - 1100)) { //900
                    ServoArm(robo.armltransfer, robo.armrtransfer);
                    ClawRot(robo.clawrotatetransfer);
                    Claw(robo.clawautograb);
                    if (runtime.milliseconds() > (drivetimeout - 500)) { //300
                ...
                droptime = runtime.milliseconds();
                drop = true;
            } else if (drop && (runtime.milliseconds() > (droptime + 400))) {
                Claw(robo.clawopenfull);

                if (runtime.milliseconds() > (droptime + 500)) {
                    Claw(robo.clawautograb);
                    ServoArm(robo.armltransfer, robo.armrtransfer);
                    ClawRot(robo.clawrotatetransfer);
                }
            ...
            if (runtime.milliseconds() > ((drivetimeout - time) + 150)) {
                if (robo.pin0.getState() && robo.pin1.getState()) {
                    robo.colorseen = "Yellow";
                } else if (robo.pin0.getState()) {
                    robo.colorseen = "Blue";
            ...
            if (robo.colorseen.matches("Red")) {
                DriveHold(true);
                DriveStop();
                Intake(robo.intakeclosed);
                needtodrive = false;
                grabtime = runtime.milliseconds();
            } else if ((!driving) && (Math.abs(robo.otos.getVelocity().x) < 0.2)) {
                robo.fl.setPower(0.44);
                robo.fr.setPower(-0.44);
        ...
        while ((runtime.milliseconds() - grabtime) < 300) {
            idle();
        }
        ...
        tradetimelimit = runtime.milliseconds();

        while (robo.clawbeam.getState() && ((runtime.milliseconds() - tradetimelimit) < 300)) {
            idle();
        }
        ...
        while ((runtime.milliseconds() - grabtime) < 150) {
            idle();
        }
    ...
    static class SampleAlignmentPipeline extends OpenCvPipeline {
        ...
        public void Equations() {
            ...
            double distance = ((0.0000000001282) * (Math.pow(reversey, 4))) - ((0.0000001066) * (Math.pow(reversey, 3))) +
                    ((0.00005792) * (Math.pow(reversey, 2))) + (0.0102 * reversey) + 4.5; //5.5 //3.9049

            if (distance < 6) {
                distance = distance - 0.5;
            } else if ((grabangle == 45) || (grabangle == (-45))) {
            ...
            double servoext = ((-0.00000161) * (Math.pow(distance, 6))) + ((0.00009517) * (Math.pow(distance, 5))) -
                    ...
                    (Math.pow(distance, 2))) + (0.6626 * distance) - 0.85; //75 //0.8491
